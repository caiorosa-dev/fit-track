...
const addExerciseLogSchema = z.object({
  sets: z.string().min(0, "Informe o número de séries").refine((value) => !isNaN(Number(value)), "Informe um número válido"),
  repetitions: z.string().min(0, "Informe o número de repetições").refine((value) => !isNaN(Number(value)), "Informe um número válido"),
  weight: z.string().min(0, "Informe o peso").refine((value) => !isNaN(Number(value)), "Informe um peso válido"),
});
...
export function AddExerciseLogDialog({ sessionId, exercise, children }: PropsWithChildren<{ sessionId: string, exercise: Exercise }>) {
  ...
  const form = useZodForm({
    ...
    handler: async (data) => {
      await api.post(`/exercise-logs`, { workoutSessionId: Number(sessionId), exerciseId: exercise.id, sets: Number(data.sets), repetitions: Number(data.repetitions), weight: Number(data.weight) });
    },
    onSubmitError: (error) => {
      console.error(error);
  ...
  return (
    <Dialog>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Cadastrar <span className="text-primary">{exercise.name}</span></DialogTitle>
          <DialogDescription>
        ...
        <Form {...form} className="space-y-6">
          <div className='grid grid-cols-2 gap-4'>
            <FormField name="sets" control={form.control} render={({ field }) => (
              <FormItem>
            ...
            <FormField name="repetitions" control={form.control} render={({ field }) => (
              <FormItem>
